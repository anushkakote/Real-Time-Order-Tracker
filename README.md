...
What happens when a client creates or updates an order:
...
sequenceDiagram
    ...
    participant SMS as Alert Service
    participant Phone as Customer Phone

    Client->>API: POST /api/orders (create order)
    API->>DB: INSERT INTO orders
    DB-->>DB: Trigger fires
    DB->>CDC: pg_notify(order_changes, row JSON)
    CDC->>Kafka: Publish to order-events topic
    Kafka->>WS: Consumer picks up event
    WS->>Client: WebSocket push (real-time UI update)
    Kafka->>SMS: Consumer picks up event
    SMS->>Phone: Send SMS via Twilio
```
## Future Improvements
